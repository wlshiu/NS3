...
#define DS_PKT_INTERVAL  0.1
#define US_PKT_INTERVAL  0.4
...
NS_LOG_COMPONENT_DEFINE ("XgponAnimationTcp");
...
main (int argc, char *argv[])
{
  ...
  std::string p_animFilename = "tcp-animation.xml";
  ...
  ApplicationContainer leftServers = leftSink.Install (leftNodes);
  ...
  for(int i=0; i<p_nOnus; i++)
  {
    ...
    sourceApps.Start (Seconds(p_appStartTime + i * 0.01));
    sourceApps.Stop (Seconds(p_appStopTime));


    BulkSendHelper source2 ("ns3::TcpSocketFactory",
                         InetSocketAddress (leftLinkInterfaces[i].GetAddress (0), leftServerPort));
    ...
    sourceApps2.Start (Seconds(p_appStartTime + i * 0.01));
  ...
  for(int i=0; i<p_nOnus; i++)
  {
    Ptr<PacketSink> sink1 = DynamicCast<PacketSink> (leftServers.Get (i));
