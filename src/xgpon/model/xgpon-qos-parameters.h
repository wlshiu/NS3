...
public:
  ...
  void SetMaxBw (uint32_t bw);
  uint32_t GetMaxBw () const;
...
private:
  ...
  uint32_t  m_maxBw;           //the maximal bandwidth.


  uint32_t  m_maxInterval;     //the maximal interval between consecutive service. Unit: nanosecond
  uint32_t  m_minInterval;     //the minimum interval between consecutive service. Unit: nanosecond
...
inline void 
XgponQosParameters::SetMaxBw (uint32_t bw)
{
  m_maxBw = bw;
}
inline uint32_t 
XgponQosParameters::GetMaxBw () const
{
  return m_maxBw;
}
