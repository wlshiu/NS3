...
namespace ns3{
    ...
    typedef enum {SLEEP,LISTEN,AWARE,REFRESH} xgPonDeviceStates;
...
public:
  ...
  EventId ScheduleState(xgPonDeviceStates state, Time schedtime);
  static void SetState(xgPonDeviceStates state, Ptr<PonNetDevice> ponDev);
...
protected:
  ...
  xgPonDeviceStates currentState;
  EventId nextStateChangeEventId;
  Time latestStateUpdate;
