...
void 
XgponOltDbaEngineRoundRobin::AddTcontToDbaEngine (const Ptr<XgponTcontOlt>& tcont)
{
...
//  uint32_t orgIndex = m_lastScheduledTcontIndex;
...
XgponOltDbaEngineRoundRobin::GetFirstTcontOlt ( )
{
  ...
  Ptr<XgponTcontOlt> tcontOlt = GetCurrentTcontOlt ( );
  if(m_getNextTcontAtBeginning == true) tcontOlt = GetNextTcontOlt ( );
  m_getNextTcontAtBeginning = true;
  NS_ASSERT_MSG((tcontOlt!=0), "There is no T-CONT in the network!!!");

  m_firstTcontOlt = tcontOlt;  //to break the following loop
...
XgponOltDbaEngineRoundRobin::CalculateAmountData2Upload (const Ptr<XgponTcontOlt>& tcontOlt,uint32_t allocatedSize, uint64_t nowNano)
{
  ...
  else
  {
    ...
    if((nowNano - lastPollingTime) > m_maxPollingInterval) size2Assign = 1;
  }
