...
XgponOltDbaEngine::XgponOltDbaEngine (): m_bursts(), m_servedBwmaps(0), m_nullBwmap(0),
    ...
    if(size2Assign > 0 && numScheduledTconts < MAX_TCONT_PER_BWMAP)
    {
      ...
      if(perBurstInfo!=0)
      {
        ...
        if(perBurstInfo->GetBwAllocNumber() == 0) 
        {
          ...
          Ptr<XgponXgtcBwAllocation> bwAlloc = Create<XgponXgtcBwAllocation> (tcontOlt->GetAllocId(), true, linkInfo->GetPloamExistAtOnu4OLT(), 0,
                                                                            size2Assign, 0, linkInfo->GetCurrentProfileIndex());
        ...
        else
        {
          ...
          {
            //Create the bwalloc
            Ptr<XgponXgtcBwAllocation> bwAlloc = Create<XgponXgtcBwAllocation> (tcontOlt->GetAllocId(), true, linkInfo->GetPloamExistAtOnu4OLT(), 0xFFFF,
                                                                            size2Assign, 0, 0);
