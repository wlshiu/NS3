...
uint16_t 
XgponHelper::AddOneTcontForOnu (Ptr<XgponOnuNetDevice> onuDevice, Ptr<XgponOltNetDevice> oltDevice)
{
  uint16_t onuId = onuDevice->GetOnuId ( );
  uint16_t allocId = m_idAllocator->GetOneNewAllocId (onuId);

  AddOneTcontForOnu (onuDevice, oltDevice, allocId);
...
void 
XgponHelper::AddOmciConnectionsForOnu (Ptr<XgponOnuNetDevice> onuDevice, Ptr<XgponOltNetDevice> oltDevice)
{
  uint16_t onuId = onuDevice->GetOnuId ( ) ;
  Address addr = onuDevice->GetAddress ( ) ;

  AddOneTcontForOnu (onuDevice, oltDevice, onuId);

  AddOneUpstreamConnectionForOnu (onuDevice, oltDevice, onuId, addr, onuId);

  AddOneDownstreamConnectionForOnu (onuDevice, oltDevice, addr, onuId);
...
XgponHelper::AddOneBroadcastDownstreamConnection (Ptr<XgponOltNetDevice> oltDevice, const Address& addr)
{
  ...
  connSender->SetQosParameters (qosParameters);
  ...
  for(int i=0; i<num; i++)
  {
    ...
    connReceiver->SetQosParameters (qosParameters2);
...
void 
XgponHelper::AddOneTcontForOnu (Ptr<XgponOnuNetDevice> onuDevice, Ptr<XgponOltNetDevice> oltDevice, uint16_t allocId)
{
  uint16_t onuId = onuDevice->GetOnuId ( );

  Ptr<XgponTcontOlt> tcontOlt = CreateObject<XgponTcontOlt> ( );
