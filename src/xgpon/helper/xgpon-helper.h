...
public:
   ...
   */
  uint16_t AddOneTcontForOnu (Ptr<XgponOnuNetDevice> onuDevice, Ptr<XgponOltNetDevice> oltDevice);
  ...
  /**
   * \brief Add OMCI channels (xgem ports and ALLOC-ID) between the OLT and one ONU
   * \param onuDevice the ONU that this OMCI channels belong to
   * \param oltDevice the OLT
   */
  void AddOmciConnectionsForOnu (Ptr<XgponOnuNetDevice> onuDevice, Ptr<XgponOltNetDevice> oltDevice);
...
private:
  ...
  void AddOneTcontForOnu (Ptr<XgponOnuNetDevice> onuDevice, Ptr<XgponOltNetDevice> oltDevice, uint16_t allocId);
